...
void InputTexto::atualizar(){
	...
	if (gTeclado.segurando[TECLA_SHIFT_ESQ]) {
		if (gTeclado.pressionou[TECLA_ESQ]) 		gTeclado.inputTexto.moverSelecaoParaEsq();		
		if (gTeclado.pressionou[TECLA_DIR]) 		gTeclado.inputTexto.moverSelecaoParaDir();		
		if (gTeclado.pressionou[TECLA_HOME])		gTeclado.inputTexto.moverSelecaoParaInicio();	
		if (gTeclado.pressionou[TECLA_END]) 		gTeclado.inputTexto.moverSelecaoParaFim();		
	}
	//		Mover o cursor do texto
	else {
		if (gTeclado.pressionou[TECLA_ESQ]) 		gTeclado.inputTexto.moverPosCursorParaEsq();		
		if (gTeclado.pressionou[TECLA_DIR]) 		gTeclado.inputTexto.moverPosCursorParaDir();		
		if (gTeclado.pressionou[TECLA_HOME])		gTeclado.inputTexto.moverPosCursorParaInicio();		
		if (gTeclado.pressionou[TECLA_END]) 		gTeclado.inputTexto.moverPosCursorParaFim();		
	}
